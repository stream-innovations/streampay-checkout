    ...
    <div class="infoBlock border-none lg:min-h-screen pt-4 pb-1 md:pt-10 md:pb-10 px-5 md:px-10">
      <header class="lg:mb-6">
        <div class="flex justify-center md:items-center mb-2">
          <a href="//streampayments.co" class="logo-container" target="_blank">
            <img src="@/static/images/logo.svg" class="StreamPayLogo" alt="Checkout by StreamPay" />
          </a>
          <div
...
import { FeeType, StreamTokenPrices } from "@matterlabs/zksync-nuxt-core/types";
...
export default Vue.extend({
  ...
  computed: {
    ...
    feeToken: {
      get(): TokenSymbol {
        return this.$store.getters["transaction/feeSymbol"];
      },
    ...
    allFees(): Array<TransactionFee> {
      if (!this.loggedIn) {
        return this.$store.getters["transaction/fees"].filter(
          (item: TransactionFee) => item.key !== "accountActivation"
        );
      }
      return this.$store.getters["transaction/fees"];
    },
    zkWalletLink(): string {
      return `//wallet.streampayments.co?network=${ETHER_NETWORK_NAME}`;
    },
    ...
    tokensPrices(): StreamTokenPrices {
      return this.$store.getters["stream-tokens/tokenPrices"];
    },
  },
  methods: {
    getFeeNameFromKey(key: FeeType) {
      if (key === "txFee") {
        return "Tx Batch Fee / StreamPay";
