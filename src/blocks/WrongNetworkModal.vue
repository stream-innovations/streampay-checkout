...
export default Vue.extend({
  ...
  computed: {
    network(): string {
      return (this.$store.getters["onboard/config"] as Config).ethereumNetwork.name;
    },
    opened: {
      set(val): void {
        if (val === false) {
          this.$store.dispatch("onboard/rejectNetworkChange");
        }
      },
      get(): boolean {
        return this.$store.getters["onboard/wrongNetwork"];
      },
    },
  },
});
