<template>
  <transition name="fade">
    <div v-if="connecting && !connectingScreenDelay" class="loggingInContainer">
      <img src="@/static/images/logo.svg" class="h-16" alt="Checkout by StreamPay" />
...
export default Vue.extend({
  ...
  computed: {
    connecting() {
      return (
        this.$store.getters["onboard/onboardStatus"] === "connecting" ||
        this.$store.getters["onboard/restoringSession"]
      );
    },
    loggedIn() {
      return this.$store.getters["onboard/onboardStatus"] === "authorized";
    },
    hintText(): string {
      return this.$store.getters["onboard/loadingHint"];
    },
    selectedWallet() {
      return this.$store.getters["onboard/selectedWallet"];
    },
  },
  watch: {
    loggingIn(val) {
