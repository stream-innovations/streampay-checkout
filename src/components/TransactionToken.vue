<template>
  <div
    ...
    :class="{ rowLayout: isInProgress || enoughStreamTokenBalance || !enoughWithInitialBalance }"
  >
    <!-- Modals -->
    <zk-modal :value="modal === 'insufficientL1Deposit' || modal === 'insufficientL1Min'" @close="modal = ''">
      ...
      <template slot="default">
        <div v-if="modal === 'insufficientL1Deposit'" class="text-sm">
          ...
          to StreamPay L2 account. Your on-chain balance is
          <strong
    ...
    <zk-line-block>
      ...
      <template slot="third">
        <div class="amount">
          <span :class="amountClass">{{ ((streamtokenBalance && streamtokenBalance.balance) || "0") | parseBigNumberish(token) }}</span>
        </div>
      </template>
      ...
      <template v-else>
        <template v-if="enoughStreamTokenBalance" slot="right">
        ...
        <template v-else slot="right">
          ...
          <div v-else-if="!enoughWithInitialBalance" class="flex items-center">
            ...
            ></i>
            <buy-with-ramp-btn
              v-if="allowedRampStreamTokens.includes(token)"
              :token="token"
              :disabled="ethereumBalanceLoading"
            />
...
import { Network, TokenSymbol } from "streampay/build/types";
import { StreamTokenBalance } from "@matterlabs/zksync-nuxt-core/types";
...
export default Vue.extend({
  ...
  computed: {
    accountStateLoaded(): boolean {
      return this.$store.getters["account/accountStateRequested"];
    },
    ethereumBalanceLoading(): boolean {
      return this.$store.getters["stream-token-balances/ethereumBalanceLoading"][this.token];
    },
    unlocked(): boolean {
      // eslint-disable-next-line no-unused-expressions
      this.$store.getters["stream-token-balances/tokensAllowanceForceUpdate"];
      return (
        this.enoughStreamTokenBalance ||
        BigNumber.from(this.$store.getters["stream-token-balances/tokenAllowance"](this.token) || "0").gte(this.needToDeposit)
      );
    },
    currentNetworkName(): Network {
      return this.$store.getters["provider/network"];
    },
    isFeeTokenLoading(): boolean {
      return (
        this.token === this.$store.getters["transaction/feeSymbol"] &&
        (this.$store.getters["transaction/feeLoading"] || this.$store.getters["transaction/activationFeeLoading"])
      );
    },
    isAllowanceLoading(): boolean {
      // eslint-disable-next-line no-unused-expressions
      this.$store.getters["stream-token-balances/tokensAllowanceForceUpdate"];
      return !!this.$store.getters["stream-token-balances/tokensAllowanceLoading"][this.token];
    },
    ...
    amountClass(): string {
      return this.enoughStreamTokenBalance ? "text-green" : "text-red";
    },
    streamtokenBalance(): StreamTokenBalance | undefined {
      return this.$store.getters["stream-token-balances/balances"][this.token];
    },
    isZkBalanceDepositing(): boolean {
      return BigNumber.from(this.$store.getters["stream-token-balances/depositingBalances"][this.token]?.amount || "0").gt("0");
    },
    confirmationsAmount(): number {
      return this.$store.getters["checkout/getConfirmationsAmount"];
    },
    initialBalance(): BigNumber {
      return BigNumber.from(this.$store.getters["stream-token-balances/ethereumBalances"][this.token] || "0");
    },
    ...
    needToDeposit(): BigNumberish {
      try {
        ...
        const recommendedAmount = BigNumber.from(this.total).sub(this.streamtokenBalance?.balance.toString() || "0");
        if (txBatchFee.token === this.token) {
          const amount = BigNumber.from(this.total)
            .sub(txBatchFee.amount)
            .add(txBatchFee.realAmount)
            .sub(this.streamtokenBalance?.balance.toString() || "0");
    ...
    enoughStreamTokenBalance(): boolean {
      return (
        BigNumber.from(this.streamtokenBalance?.balance || "0").gte(this.total) || BigNumber.from(this.needToDeposit).lte("0")
      );
    },
    ...
    enoughWithInitialBalance(): boolean {
      return BigNumber.from(this.streamtokenBalance?.balance || "0")
  ...
  watch: {
    ...
    step(val) {
      this.$emit("input", this.enoughStreamTokenBalance && val === "default");
    },
    isInProgress(val, oldVal) {
      if (!val && oldVal && (!this.depositAmount || this.depositAmount === "0") && !this.enoughStreamTokenBalance) {
        this.setDepositRecommendedAmount();
      }
    },
  },
  created() {
    if (!this.enoughStreamTokenBalance) {
      this.setDepositRecommendedAmount();
    }
  },
  methods: {
    refreshBalance() {
      this.$store.dispatch("stream-token-balances/requestEthereumBalance", { symbol: this.token, force: true });
      this.$store.dispatch("account/updateAccountState");
    },
    setDepositMaxAmount() {
    ...
    async deposit() {
      if (!this.enoughOnInitialToDeposit) {
        ...
        try {
          ...
          const depositResponse = await streamWallet.depositToSyncFromEthereum({
            depositTo: streamWallet.address(),
            token: this.token,
          ...
          const dataPromises = [
            this.$store.dispatch("stream-token-balances/requestEthereumBalance", { force: true, symbol: "ETH" }),
            this.$store.dispatch("account/updateAccountState", true),
          ];
          if (this.token !== "ETH") {
            dataPromises.push(
              this.$store.dispatch("stream-token-balances/requestAllowance", { force: true, symbol: this.token })
            );
            dataPromises.push(
              this.$store.dispatch("stream-token-balances/requestEthereumBalance", { force: true, symbol: this.token })
            );
          }
    ...
    async unlock() {
      try {
        ...
        this.step = "unlocking";
        const streamWallet: Wallet = this.$store.getters["wallet/streamWallet"];
        const tokenAddress = streamWallet.provider.tokenSet.resolveTokenAddress(this.token);
        const unlockTransaction = await streamWallet.approveERC20TokenDeposits(tokenAddress);
        this.subStep = "committing";
