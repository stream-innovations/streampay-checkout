...
import { StreamTokenBalances } from "@matterlabs/zksync-nuxt-core/types";
import { StreamSingleToken, StreamTokens } from "@/types";
...
export default Vue.extend({
  ...
  computed: {
    tokens(): StreamTokens {
      return this.$store.getters["stream-tokens/zkTokens"] as StreamTokens;
    },
    displayedTokens(): StreamSingleToken[] {
      const result: StreamSingleToken[] = [];
      let key: string;
      for (key in this.tokens) {
        if (
          ...
          result.push(this.tokens[key] as StreamSingleToken);
        }
      }
      if (this.$store.getters["account/loggedIn"]) {
        const balances: StreamTokenBalances = this.$store.getters["stream-token-balances/balances"];
  ...
  watch: {
    displayedTokens: {
      deep: true,
      handler(tokens: StreamSingleToken[]) {
        if (tokens.filter((singleToken: StreamSingleToken) => this.value === singleToken.symbol).length === 0) {
  ...
  methods: {
    ...
    setToken(token: StreamSingleToken, index: number = 0): void {
