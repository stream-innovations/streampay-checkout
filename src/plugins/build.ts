import { Network } from "streampay/build/types.d";
import { version as streamPayVersion } from "streampay/package.json";
...
export const LIB_VERSION = streamPayVersion ?? "latest";
export const ETHER_NETWORK_CAPITALIZED = `${ETHER_NETWORK_NAME.charAt(0).toUpperCase()}${ETHER_NETWORK_NAME?.slice(1)}`;
export const CURRENT_APP_NAME = "Stream Checkout";

export const CURRENT_APP_TITLE = process.env.SITE_TITLE || CURRENT_APP_NAME;

export const STREAM_NETWORK: string = process.env.STREAM_NETWORK ? process.env.STREAM_NETWORK : ETHER_NETWORK_NAME;
...
export const rampConfig: RampConfig = {
  mainnet: {
